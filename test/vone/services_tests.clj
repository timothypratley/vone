...
(facts "about private functions"
       (fact (#'vone.services/take-to 2 [1 2 3]) => [1 2])
       (fact (#'vone.services/take-after 2 [1 2 3]) => [3])
       (fact (#'vone.services/take-before 2 [1 2 3]) => [1])
       (fact (#'vone.services/roadmap-transform
              [["5.0.0" "CSX" "TC" :a 1]
               ["5.0.0" "CSX" "TC" :a 2]
               ["5.0.0" "CSX" "SP" :a 3]
               ["5.0.0" "CSX" "SP" :a 4]])
...
(facts "about services"
       (fact (#'vone.services/compress [:a :a :b :c :a :c :c :c]) => [:a :b :c :a :c])
       (fact (#'vone.services/compress [{:a 1} {:a 2} {:a 2}] :a) => [{:a 1} {:a 2}])
       (fact (#'vone.services/index [{:a :A, :b 2} {:a :B :b 3}] :a) => {:A {:a :A, :b 2} :B {:a :B, :b 3}})
       (fact (#'vone.services/map-difference {:a 1 :b 2} {:b 3}) => {:a 1})
       ...
       (fact (#'vone.services/todo-on "TC Sharks" "TC1315" (time/now)) => number?)
